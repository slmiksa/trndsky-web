...
const AdminDashboard = () => {
  ...
  return <SidebarProvider defaultOpen={!isMobile}>
      <div className="min-h-screen bg-gray-50 flex flex-col">
        {isMobile && <div className="bg-gray-100 p-4 flex justify-between items-center border-b">
            ...
            <SidebarContent>
              <SidebarMenu>
                {menuItems.map(item => <SidebarMenuItem key={item.id}>
                    <SidebarMenuButton isActive={activeTab === item.id} onClick={() => {
                  setActiveTab(item.id as AdminTab);
                  if (isMobile) {
                    setSidebarOpen(false);
                  }
                }} className="justify-end font-tajawal rounded-none text-base font-semibold text-trndsky-blue text-center">
                      {item.label}
                    </SidebarMenuButton>
                  </SidebarMenuItem>)}
              </SidebarMenu>
